...
Request *http_parse_request(int clientfd) {
    ...
    if (received_length < 0) {
        ...
        request->payload.content_length = content_length_header_value ? atol(content_length_header_value) : (
                received_length - (request->payload.content - request->buffer));
...
void http_send_response(Request *request, Response *response) {
    ...
    */
    ...
    while (header < response->headers + response->header_count) {
        n = snprintf(NULL, header_structure, header->name, header->value);
        total_char += n;

        headers_buffer = realloc(headers_buffer, total_char);
        header_buffer = malloc(n);
    ...
    if (headers_buffer != NULL && response->header_count > 0) {
        ...
        response_content = realloc(response_content, response_size);
    ...
    if (send(request->clientfd, response_content, response_size, 0) == -1) {
